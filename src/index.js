...
function shimModel(target) {
  ...
  shimmer.massWrap(target, methods(Sequelize.version).findByPk, (original) => {
    return function batchedFindById(id, options = {}) {
      ...
      return Promise.resolve(loader.load(id)).then(
        rejectOnEmpty.bind(null, options)
      );
    };
  });
}
    ...
    return function batchedGetBelongsTo(instance, options = {}) {
      ...
      return Promise.resolve()
        .then(() => {
          ...
          const loader =
            options[EXPECTED_OPTIONS_KEY].loaders[this.target.name]
              .bySingleAttribute[this.targetKey];
...
function shimHasOne(target) {
  ...
  shimmer.wrap(target, "get", (original) => {
    return function batchedGetHasOne(instance, options = {}) {
      ...
      const loader =
        options[EXPECTED_OPTIONS_KEY].loaders[this.target.name]
          .bySingleAttribute[this.foreignKey];
      return Promise.resolve(
        loader
          .load(instance.get(this.sourceKey))
          .then(rejectOnEmpty.bind(null, options))
      );
    };
  });
...
function shimHasMany(target) {
  ...
  shimmer.wrap(target, "get", (original) => {
    return function batchedGetHasMany(instances, options = {}) {
      ...
      const useCache = options[EXPECTED_OPTIONS_KEY].cache;
      if (useCache) {
        cacheKey = getCacheKey(this.target, this.foreignKey, loaderOptions);
        loader = options[EXPECTED_OPTIONS_KEY].loaders.autogenerated.get(
          cacheKey
        );
      }
      if (!loader) {
...
export function createContext(sequelize, options = { cache: true }) {
  const loaders = {};

  shimModel(
    /^[45]/.test(sequelize.constructor.version) // v3 vs v4
      ? sequelize.constructor.Model
      : sequelize.constructor.Model.prototype
  );
  ...
  for (const Model of Object.values(sequelize.models)) {
    ...
    loaders[Model.name].bySingleAttribute[
    ...
    values(Model.associations).forEach((association) => {
      if (association.associationType === "BelongsTo") {
        ...
        if (association.targetKey !== Target.primaryKeyAttribute) {
          loaders[Target.name].bySingleAttribute[
            association.targetKey
          ] = createModelAttributeLoader(
            Target,
            association.targetKey,
            options
          );
  ...
  return { loaders, prime, cache: options.cache };
}
